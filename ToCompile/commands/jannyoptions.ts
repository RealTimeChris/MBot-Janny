...
async function execute(message: Discord.Message, args: string[], discordUser: DiscordStuff.DiscordUser): Promise<string> {
	try {
		...
		msgEmbed.setAuthor((message.client.user as Discord.User).username, (message.client.user as Discord.User).avatarURL() as string).setTimestamp((Date() as unknown) as Date).setTitle('__**Janny Options:**__').setColor([0, 0, 255])

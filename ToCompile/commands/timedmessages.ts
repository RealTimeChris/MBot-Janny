...
async function execute(commandData: FoundationClasses.CommandData, discordUser: DiscordUser): Promise<FoundationClasses.CommandReturnData> {
    try {
        ...
        if (whatAreWeDoing === 'viewing') {
            ...
            for (let x = 0; x < guildData.exposeDataValues().timedMessages!.length; x += 1) {
                ...
                const timeRemaining = guildData.exposeDataValues().timedMessages![x]!
                ...
                const currentField = { name: `__**${guildData.exposeDataValues().timedMessages![x]!.name}:**__`, value: `__**ms Between Sends:**__ 
        ...
        if (whatAreWeDoing === 'removing') {
            ...
            for (let x = 0; x < guildData.exposeDataValues().timedMessages!.length; x += 1) {
                if (messageName === guildData.exposeDataValues().timedMessages![x]!.name) {
                    isItFound = true;

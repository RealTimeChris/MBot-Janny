// ghost.js - Module for my "ghost" command.
// Mar 18, 2021
// Chris M.
	...
	async execute(message, args, discordUser) {
		try {
			...
			+ '@USERMENTION to ghost a new user, !ghost = remove, @USERMENTION to unghost a user)');
				await message.delete();
				return this.name;
			} else if (args[0] !== undefined && args[0].toLowerCase() === 'add' && !userIDRegExp.test(args[2])) {
				await message.reply('Please, enter a proper usermention to select the target to ghost! (!ghost = add, REASON,'
			+ '@USERMENTION to ghost a new user, !ghost = remove, @USERMENTION to unghost a user)');
				await message.delete();
				return this.name;
			} else if (args[0] !== undefined && args[0].toLowerCase() === 'remove' && !userIDRegExp.test(args[1])) {
			...
			const currentGuildMember = await message.guild.members.fetch(userID);

			const guildMemberData = await discordUser.getGuildMemberDataFromDB(currentGuildMember);

			const channelsArray = (await message.client.guilds.fetch(message.guild.id))
				.channels.cache.array();

			const roleManager = new Discord.RoleManager(message.guild);
			...
			if (whatAreWeDoing === 'add' || whatAreWeDoing === 'remove') {
				for (let x = 0; x < channelsArray.length; x += 1) {
					...
					voicePermissionOptions.channel = '';
					...
					if (channelsArray[x].type === 'voice') {
						let currentChannel = new Discord.VoiceChannel(message.guild, {});

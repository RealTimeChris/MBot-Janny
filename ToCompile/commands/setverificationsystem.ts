...
async function execute(commandData: DiscordStuff.CommandData,  discordUser: DiscordStuff.DiscordUser): Promise<DiscordStuff.CommandReturnData> {
    try {
        ...
        if (whatAreWeDoing === 'disable') {
            const currentChannel = (commandData.guildMember as Discord.GuildMember).client.channels
        ...
        if (whatAreWeDoing === 'enable') {
            ...
            if (commandData.toTextChannel instanceof Discord.WebhookClient){
                newMessage = new Discord.Message((commandData.guild as Discord.Guild).client, newMessage, commandData.fromTextChannel as Discord.TextChannel) as Discord.Message;
            }            
            
            await newMessage.react(commandData.args[2] as string);

            guildData.verificationSystem.channelID = (commandData.fromTextChannel as Discord.TextChannel).id;
